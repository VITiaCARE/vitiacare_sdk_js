const { vitiaObject, Intake } = require('./single');
...
class Intakes extends vitiaMultiObject {
  ...
  async confirm(mode = true, id = [], index = []) {
    if (id === [] && index === []) id = this.value.map((v) => v._key);
    let id_ans = [];
    let index_ans = [];
    let intake = new Intake(this.getStore());
    await intake.prepare();
    for (let i = 0; i < id.length; i++) {
      try {
        id_ans.push(await intake.confirm(id[i], mode))
      } catch {
        console.log("error confirmation")
        id_ans.push({ error: true, error_desc: 'error when confirming', err_code: this.error_codes.MISSING_SEARCH_ID })
      }
    }
    for (let j = 0; j < index.length; j++) {
      try {
        index_ans.push(await intake.confirm(this.value[index[j]]._key, mode))
      } catch {
        console.log("error confirmation")
        id_ans.push({ error: true, error_desc: 'error when confirming', err_code: this.error_codes.MISSING_SEARCH_ID })
      }
    }
    return { id_res: id_ans, index_res:index_ans};
  }
}
