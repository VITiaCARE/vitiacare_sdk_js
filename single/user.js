...
export class User extends vitiaObject {
    ...
    async get_profile_pic_url(pic_id = null) {
      let file_id = (pic_id === null) ? this.value.profile_pic : pic_id;
      await this.send_request('GET', `user/file/uri/${file_id}`);    
      switch (this.response.status) {
        case 200:
          this.response.error = false
          return this.response.json().then(async (data) => {
